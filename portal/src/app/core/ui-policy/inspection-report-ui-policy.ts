...
export function getInspectionReportUiState(
  ...
  if (ctx.reportStatus === REPORT_STATUSES.ON_HOLD) {
    state.banners.push({
      type: 'warning',
      message: `On Hold — reason: ${ctx.onHoldReason || 'Unknown'}, previous status: ${ctx.previousStatus || 'Unknown'}`,
    });
  }
  ...
  if (ctx.reportStatus === REPORT_STATUSES.CLOSED) {
    state.banners.push({
      type: 'info',
      message: 'Locked — Closed (exportable).',
    });
  }
  ...
  if (
    [APP_ROLES.INSPECTOR, APP_ROLES.SUPERVISOR, APP_ROLES.ADMIN].some(
      (r) => r === ctx.role,
    )
  ) {
  ...
  if (ctx.syncState !== 'CONFLICT') {
    if (ctx.reportStatus === REPORT_STATUSES.DRAFT) {
      ...
      if (ctx.role === APP_ROLES.INSPECTOR || ctx.role === APP_ROLES.ADMIN) {
        ...
        state.actions['SN_EDIT_INSPECTION_DATA'] = {
          visible: true,
          enabled: true,
        };
        state.actions['SN_SET_DISPOSITION'] = { visible: true, enabled: true };
        state.actions['CR_CREATE_FROM_REWORK'] = {
          visible: true,
          enabled: true,
        };

        if (ctx.role === APP_ROLES.INSPECTOR) {
          state.actions['IR_EDIT_SERIAL'] = {
            visible: true,
            enabled: true,
            requiresReason: true,
          };
  ...
  if (
    [APP_ROLES.CUSTOMER, APP_ROLES.ADMIN, APP_ROLES.SUPERVISOR].some(
      (r) => r === ctx.role,
    )
  ) {
    const isApprovedOrClosed =
      ctx.reportStatus === REPORT_STATUSES.APPROVED ||
      ctx.reportStatus === REPORT_STATUSES.CLOSED;
    const isExportEnabled = isApprovedOrClosed && !ctx.isOffline;
    const disabledReason = !isApprovedOrClosed
      ? 'Only available when Approved or Closed.'
      : ctx.isOffline
        ? 'Export requires internet connection.'
        : undefined;

    const key =
      ctx.role === APP_ROLES.CUSTOMER ? 'CUSTOMER_EXPORT' : 'ADMIN_EXPORT';
    state.actions[key] = {
      visible: true,
  ...
  if (ctx.syncState !== 'CONFLICT') {
    if (ctx.reportStatus === REPORT_STATUSES.DRAFT) {
      ...
      if ([APP_ROLES.SUPERVISOR, APP_ROLES.ADMIN].some((r) => r === ctx.role)) {
        state.transitionChoices.push({
          toStatus: REPORT_STATUSES.ON_HOLD,
          label: 'Hold',
          requiresReason: true,
          enabled: true,
        });
      ...
      if (
        [APP_ROLES.RECEIVER, APP_ROLES.SUPERVISOR, APP_ROLES.ADMIN].some(
          (r) => r === ctx.role,
        )
      ) {
        state.transitionChoices.push({
          toStatus: REPORT_STATUSES.READY_FOR_CLEANING,
      ...
      if (
        ...
        state.transitionChoices.push({
          toStatus: REPORT_STATUSES.READY_FOR_INSPECTION,
      ...
      if ([APP_ROLES.SUPERVISOR, APP_ROLES.ADMIN].some((r) => r === ctx.role)) {
        state.transitionChoices.push({
          ...
          label: 'Approve',
          requiresReason: false,
          enabled: !ctx.hasValidationIssues,
          disabledReason: ctx.hasValidationIssues
            ? 'Validation failed.'
            : undefined,
        });
        state.transitionChoices.push({
      ...
      if ([APP_ROLES.SUPERVISOR, APP_ROLES.ADMIN].some((r) => r === ctx.role)) {
        state.transitionChoices.push({
          toStatus: ctx.previousStatus || REPORT_STATUSES.DRAFT,
          label: 'Release Hold',
          requiresReason: false,
          enabled: true,
        });
