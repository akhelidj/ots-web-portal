 ...
 * exact same code. Nothing here knows about drill pipe: the marker, the per-row
 * token keys, the global token map, and the per-serial token map are all
 * parameters. Behavior is byte-preserving relative to the pre-B2 mapper (pinned
 * by export.integration.spec.ts + the B2 equivalence specs).
 */
...
export interface RegionSubstitution {
  /** The token whose worksheet row is the clone template (e.g. `{{sn}}`). */
  marker: string;
  /** The full set of per-row token keys used to detect token-bearing cells. */
  rowTokenKeys: string[];
...
/**
 * Expand the marker row once per item and substitute both per-row and global
 * tokens, in place, on `workbook`. This is steps 2–8 of the original
 * `mapDrillPipeReportV1`, verbatim, parameterized by `opts`.
 */
export async function expandRegionAndSubstitute(
  ...
  const { marker, rowTokenKeys, globalTokens, rowTokensFor } = opts;
  ...
  // 4. Find the template row that contains the marker by checking shared string values
  ...
  while ((rowMatch = rowRegex.exec(sheetXml)) !== null) {
    ...
    for (const idx of indices) {
      if (ssPlain[idx] && ssPlain[idx].includes(marker)) {
  ...
  // 5. Handle repeated-row expansion (if the marker template row was found)
